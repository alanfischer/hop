...
// test_inside
template<typename T>
inline bool test_inside(const plane<T>& p, const vec3<T>& point) {
	return dot(point, p.normal) <= p.distance;
}

template<typename T>
inline bool test_inside(const sphere<T>& s, const vec3<T>& point) {
	return length_squared(point, s.origin) <= square(s.radius);
}

template<typename T>
inline bool test_inside(const aa_box<T>& box, const vec3<T>& point) {
...
// test_intersection
template<typename T>
inline bool test_intersection(const aa_box<T>& a, const aa_box<T>& b) {
	return !(a.mins.x > b.maxs.x || a.mins.y > b.maxs.y || a.mins.z > b.maxs.z ||
	         b.mins.x > a.maxs.x || b.mins.y > a.maxs.y || b.mins.z > a.maxs.z);
}
...
inline T find_intersection(const segment<T>& seg, const plane<T>& p, vec3<T>& point, vec3<T>& normal) {
	...
	if (d != zero_val) {
		...
		normal = p.normal;
		if (t < zero_val || t > tr::one()) return tr::one();
		return t;
	}
	return tr::one();
}
...
inline T find_intersection(const segment<T>& seg, const aa_box<T>& box, vec3<T>& point, vec3<T>& normal) {
	...
	// X
	if (so.x <= bmn.x) { qx = 0; cand_x = bmn.x; inside = false; }
	else if (so.x >= bmx.x) { qx = 1; cand_x = bmx.x; cnorm_x = tr::one(); inside = false; }
	else { qx = 2; }
	// Y
	if (so.y <= bmn.y) { qy = 0; cand_y = bmn.y; inside = false; }
	else if (so.y >= bmx.y) { qy = 1; cand_y = bmx.y; cnorm_y = tr::one(); inside = false; }
	else { qy = 2; }
	// Z
	if (so.z <= bmn.z) { qz = 0; cand_z = bmn.z; inside = false; }
	else if (so.z >= bmx.z) { qz = 1; cand_z = bmx.z; cnorm_z = tr::one(); inside = false; }
	else { qz = 2; }

	if (inside) return zero_val;

	if (qx != 2 && sd.x != zero_val) max_tx = (cand_x - so.x) / sd.x;
	else max_tx = neg_one;
	if (qy != 2 && sd.y != zero_val) max_ty = (cand_y - so.y) / sd.y;
	else max_ty = neg_one;
	if (qz != 2 && sd.z != zero_val) max_tz = (cand_z - so.z) / sd.z;
	else max_tz = neg_one;

	if (max_tx > max_ty && max_tx > max_tz) {
		which_plane = 0; time = max_tx;
		normal.set(cnorm_x, zero_val, zero_val);
	} else if (max_ty > max_tz) {
		which_plane = 1; time = max_ty;
		normal.set(zero_val, cnorm_y, zero_val);
	} else {
		which_plane = 2; time = max_tz;
		normal.set(zero_val, zero_val, cnorm_z);
	}

	if (time < zero_val || time > tr::one()) return tr::one();

	if (which_plane != 0) {
		point.x = so.x + time * sd.x;
		if (point.x < bmn.x || point.x > bmx.x) return tr::one();
	} else { point.x = cand_x; }
	if (which_plane != 1) {
		point.y = so.y + time * sd.y;
		if (point.y < bmn.y || point.y > bmx.y) return tr::one();
	} else { point.y = cand_y; }
	if (which_plane != 2) {
		point.z = so.z + time * sd.z;
		if (point.z < bmn.z || point.z > bmx.z) return tr::one();
	} else { point.z = cand_z; }
