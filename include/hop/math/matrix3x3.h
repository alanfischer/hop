...
// r = m1 * m2
template <typename T> inline void mul(matrix3x3<T> & r, const matrix3x3<T> & m1, const matrix3x3<T> & m2) {
...
// r = m * v
template <typename T> inline void mul(vec3<T> & r, const matrix3x3<T> & m, const vec3<T> & v) {
	// Assumes &r != &v.
...
// r = transpose(m)
template <typename T> inline void transpose(matrix3x3<T> & r, const matrix3x3<T> & m) {
...
template <typename T> inline T determinant(const matrix3x3<T> & m) {
...
template <typename T> inline bool invert(matrix3x3<T> & r, const matrix3x3<T> & m) {
	using tr = scalar_traits<T>;
	T det = determinant(m);
	if (det == T {})
		return false;
...
template <typename T> inline void set_matrix_from_axis_angle(matrix3x3<T> & r, const vec3<T> & axis, T angle) {
	using tr = scalar_traits<T>;
	T c = tr::cos(angle);
	T s = tr::sin(angle);
	T t = tr::one() - c;
