...
class HopSimulator {
	...
	bool valid(int id) const { return id >= 0 && id < static_cast<int>(solids_.size()); }

public:
	HopSimulator() = default;

	void set_gravity(float x, float y, float z) { sim_.set_gravity(hop::vec3<float>(x, y, z)); }

	void update(int dt_ms) { sim_.update(dt_ms); }
	...
	int add_box(float mass, float hx, float hy, float hz) {
		...
		s->set_mass(mass);
		s->add_shape(std::make_shared<hop::shape<float>>(
