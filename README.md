...
It is deliberately **not yet** implemented — but it is more feasible for hop than for most engines, because hop already owns the expensive half. The swept query that answers "where would I go and what would I hit on the way" is exactly what speculative contacts need; most engines have to bolt continuous collision *on* to get it, whereas here it is the core primitive. Replacing the per-tick TOI *snap* with a velocity *clamp* (remove only enough approach speed to close the remaining gap) is still pure arithmetic, so it does **not** threaten the fixed-point determinism. The real cost is reworking the tick's control flow — gather predicted contacts → solve velocities → then integrate, instead of integrate → snap → solve — and reformulating the contact target from a post-overlap fix into a pre-move gap clamp, plus re-tuning everything balanced around the old behavior. That rework, not technical risk, is why it is deferred against the translation-only game-object use case, which rarely needs thousand-body piles to fully sleep. See `docs/pipeline_reorder_plan.md` for the full implementation plan.
