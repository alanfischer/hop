...
template <typename T> static void test_point_in_box() {
	using tr = scalar_traits<T>;
	aa_box<T> box { vec3<T> { -tr::one(), -tr::one(), -tr::one() }, vec3<T> { tr::one(), tr::one(), tr::one() } };
	vec3<T> inside { T {}, T {}, T {} };
...
template <typename T> static void test_box_box_intersection() {
	using tr = scalar_traits<T>;
	aa_box<T> a { vec3<T> { -tr::one(), -tr::one(), -tr::one() }, vec3<T> { tr::one(), tr::one(), tr::one() } };
	aa_box<T> b { vec3<T> { T {}, T {}, T {} }, vec3<T> { tr::two(), tr::two(), tr::two() } };
	aa_box<T> c { vec3<T> { tr::three(), tr::three(), tr::three() }, vec3<T> { tr::four(), tr::four(), tr::four() } };
...
template <typename T> static void test_ray_box() {
	using tr = scalar_traits<T>;
	aa_box<T> box { vec3<T> { -tr::one(), -tr::one(), -tr::one() }, vec3<T> { tr::one(), tr::one(), tr::one() } };
...
template <typename T> static void test_ray_sphere() {
	using tr = scalar_traits<T>;
